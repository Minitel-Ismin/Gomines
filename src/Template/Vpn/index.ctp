<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('Télécharger la configuration', ['action' => 'dlConfig']) ?> </li>
        <?= ($user['droits'] & 2) ? "<li>".$this->Html->link('Statut du VPN',['action' => 'vpnStatus'])."</li>" : "" ?>
        <?= ($user['droits'] & 2) ? "<li>".$this->Html->link('Utilisateurs du VPN',['action' => 'vpnUsers'])."</li>" : "" ?>
        <li><?= $this->Html->link(__('Retour'), ['controller' => 'pages', 'action' => 'index']) ?> </li>
    </ul>
</nav>
